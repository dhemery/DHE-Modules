...
public:
  Generating(std::function<void()> finishGenerating, std::function<float()> duration, std::function<void()> prepare,
             std::function<void(float)> generate, std::function<void(bool)> setActive) :
      StageState{
      ...
      generator{std::move(duration), []() {}, std::move(generate), std::move(finishGenerating)} {}
