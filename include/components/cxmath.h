...
static constexpr auto pi{3.14159265359F}; //NOLINT
 ...
 */
namespace cx {
template <typename T> static constexpr auto max(T a, T b) -> T { //NOLINT
  return a > b ? a : b;
}

template <typename T> static constexpr auto min(T a, T b) -> T { //NOLINT
  return a < b ? a : b;
}

template <typename T> static constexpr auto abs(T t) -> T { //NOLINT
  return t < 0 ? -t : t;
}
