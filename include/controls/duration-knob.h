...
class DurationKnob : Knob {
  public:
    DurationKnob(const rack::Module *module, int knob_index) : Knob{module, knob_index} {}

    DurationKnob(const rack::Module *module, int knob_index, int cv_index, int range_switch_index)
        : Knob{module, knob_index, cv_index}, range_switch{&module->params[range_switch_index]} {}
  ...
  private:
    ...
    auto range() const -> const Range &{
      ...
      if(position > 1.5f) return long_range;
      if(position < 0.5f) return short_range;
      return medium_range;
    }
