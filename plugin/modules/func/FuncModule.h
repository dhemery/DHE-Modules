...
namespace func {
  ...
  public:
    ...
    void configChannel(int channel) {
      ...
      configParam<OperandParamQuantity<Controls>>(Controls::AmountKnob + channel, 0.F, 1.F, centeredRotation);
      configToggle<2>(this, Controls::OperationSwitch + channel, "Operator" + channelName,
                      {"Add (offset)", "Multiply (scale)"}, 0);
      configToggle<4>(this, Controls::OffsetRangeSwitch + channel, "Offset" + channelName + " range",
                      {"0–5 V", "±5 V", "0–10 V", "±10 V"}, 1);
      configToggle<4>(this, Controls::MultiplierRangeSwitch + channel, "Multiplier" + channelName + " range",
                      {"0–1", "±1", "0–2", "±2"}, 2);
