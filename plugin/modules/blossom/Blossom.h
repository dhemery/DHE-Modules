...
namespace blossom {
  ...
  public:
    Blossom() {
      ...
      configKnob(this, Controls::PhaseOffsetKnob, "Phase", "°", phaseDisplayRange);
  ...
  private:
    inline auto ratio() const -> float {
      return ratioRange.scale(
          rotation(params[Controls::RatioKnob], inputs[Controls::RatioCvInput], params[Controls::RatioAvKnob]));
    }

    inline auto ratioIsFree() const -> bool { return positionOf(params[Controls::FreeRatioSwitch]) == 1; }

    inline auto depth() const -> float {
      return rotation(params[Controls::DepthKnob], inputs[Controls::DepthCvInput], params[Controls::DepthAvKnob]);
    }

    inline auto phaseOffset() const -> float {
      static auto constexpr phaseRange = Range{-0.5F, 0.5F};
      return phaseRange.scale(rotation(params[Controls::PhaseOffsetKnob], inputs[Controls::PhaseCvInput],
    ...
    inline auto xOffset() const -> float { return selected<float, 2>(params[Controls::XRangeSwitch], {0.F, 1.F}); };

    inline auto yGain() const -> float {
      return gainRange.scale(rotation(params[Controls::YGainKnob], inputs[Controls::YGainCvInput]));
    }
    inline auto yOffset() const -> float { return selected<float, 2>(params[Controls::YRangeSwitch], {0.F, 1.F}); };
