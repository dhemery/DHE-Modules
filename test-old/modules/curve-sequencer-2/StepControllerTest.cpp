...
struct Generator {
  std::function<void(int)> start = [](int s) {
    throw "start " + std::to_string(s);
  };
  std::function<bool(float)> generate{
      [](float t) -> bool { throw "generate " + std::to_string(t); }};
  std::function<void()> stop = []() { throw "stop"; };
};

struct Interrupter {
  std::function<bool(int, Latch const &)> isInterrupted{
      [](int s, Latch const &l) -> bool {
        throw "isInterrupted " + std::to_string(s) + ", " + l.str();
      }};
};
...
struct Sustainer {
  std::function<bool(int, Latch const &)> isDone{
      [](int s, Latch const &l) -> bool {
        throw "isDone" + std::to_string(s) + ", " + l.str();
      }};
};
  ...
  SUBCASE("execute") {
    SUBCASE("if interrupted") {
      SUBCASE("completes without generating") {
        interrupter.isInterrupted = [](int /**/, Latch const & /**/) -> bool {
          return true;
        };
        generator.generate = [](float t) -> bool {
          throw std::string{"generate "} + std::to_string(t);
        };
    ...
    SUBCASE("if not interrupted") {
      ...
      SUBCASE("generates") {
        sustainer.isDone = [](int /**/, Latch const & /**/) -> bool {
          return false;
        };
    ...
    SUBCASE("if generator does not finish") {
      interrupter.isInterrupted = [](int /*s*/, Latch const & /*l*/) -> bool {
        return false;
      };
