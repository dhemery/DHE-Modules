...
struct GeneratorControls {
  std::function<void(int, float)> showProgress{[](int s, float p) {
    throw "showProgress " + std::to_string(s) + " " + std::to_string(p);
  }};
  std::function<void(int)> showInactive{
      [](int s) { throw "showInactive " + std::to_string(s); }};
};

struct GeneratorSource {
  std::function<void(int)> snap = [](int s) {
    throw "snap " + std::to_string(s);
  };
};
...
TEST_CASE("curve_sequencer_2::Generator") {
  ...
  std::function<void(int)> givenCurrentStep = [&](int s) {
    setCurrentStep(generator, controls, startSource, endSource, s);
  };
  ...
  SUBCASE("stop") {
    SUBCASE("deactivates the current step") {
      ...
      controls.showInactive = [&deactivatedStep](int s) {
        deactivatedStep = s;
      };
