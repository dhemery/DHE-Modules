...
UpstageWidget::UpstageWidget() : Widget(new Upstage(), 4, "res/Upstage.svg") {
  ...
  install_light<rack::MediumLight<rack::GreenLight>>(Upstage::WAIT_BUTTON_LIGHT, left_x, upper_top_y + upper_spacing);
  install_button<rack::LEDButton>(Upstage::TRIG_BUTTON, right_x, upper_top_y + upper_spacing);
  install_light<rack::MediumLight<rack::GreenLight>>(Upstage::TRIG_BUTTON_LIGHT, right_x, upper_top_y + upper_spacing);

  float lower_top_y = 245.0f;
  float lower_spacing = 43.0f;

  install_input<rack::PJ301MPort>(Upstage::WAIT_INPUT, left_x, lower_top_y);

  install_input<rack::PJ301MPort>(Upstage::TRIG_INPUT, left_x, lower_top_y + lower_spacing);
  install_output<rack::PJ301MPort>(Upstage::TRIG_OUTPUT, right_x, lower_top_y + lower_spacing);

  install_output<rack::PJ301MPort>(Upstage::OUT_OUTPUT, right_x, lower_top_y + 2 * lower_spacing);
