  ...
  public:
    Gator() {
      ...
      for (auto i = 0; i < Controls::inputCount; i++) {
        configParam(Controls::NegateButtons + i, 0.F, 1.F, 0.F, "Negate input " + std::to_string(i + 1));
      }
    }
    ...
    void process(ProcessArgs const & /*ignored*/) override {
      ...
      for (auto i = 0; i < Controls::inputCount; i++) {
        if (inputs[i].isConnected()) {
          connectedCount++;
          if (isHigh(inputs[(Controls::Inputs + i)]) != isPressed(params[Controls::NegateButtons + i])) {
            highCount++;
          }
        }
      }
      ...
      if (connectedCount == 0) {
        ...
        setOutputs(Controls::AndOutput, Controls::NandOutput, highCount == connectedCount);
        setOutputs(Controls::OrOutput, Controls::NorOutput, highCount > 0);
        setOutputs(Controls::OddOutput, Controls::EvenOutput, (highCount & 1) > 0);
