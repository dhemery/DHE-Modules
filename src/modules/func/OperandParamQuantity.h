...
public:
  ...
  void configure(Controls const *theControls, int theChannel,
                 const std::string &theChannelName) {
...
private:
  auto currentOperandRange() -> Range const * {
    return operation() == Operation::Multiply ? multiplierRange()
                                              : offsetRange();
  }
  ...
  auto multiplierRange() const -> Range const * {
    return controls->multiplierRange(channel);
  }

  auto offsetRange() const -> Range const * {
    return controls->offsetRange(channel);
  }
