...
public:
  explicit Func1Panel(rack::engine::Module *module) {
    ...
    auto *offsetRangePickList =
        picklist::button(slug, "offset-range", offsetRanges.size(), module, x,
                         row4, Controls::OffsetRangeSwitch);
    addParam(offsetRangePickList);

    auto *multiplierRangePickList =
        picklist::button(slug, "multiplier-range", multiplierRanges.size(),
                         module, x, row4, Controls::MultiplierRangeSwitch);
    addParam(multiplierRangePickList);

    auto const updateRangeStepperVisibility =
    ...
    addParam(new OperatorSwitch{updateRangeStepperVisibility, slug, module, x,
                                row2, Controls::OperationSwitch});
