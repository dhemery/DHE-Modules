...
namespace func {
  ...
  static auto constexpr offsetRanges = std::array<Range const *, 4>{&zeroToFiveRange, &bipolarSignalRange,
  ...
  public:
    FuncControls(std::vector<rack::engine::Input> const &inputs, std::vector<rack::engine::Param> const &params,
    ...
    auto input(int channel, float voltageIfDisconnected) const -> float {
      return const_cast<rack::engine::Input &>(inputs[FuncInput + channel]).getNormalVoltage(voltageIfDisconnected);
    }

    auto multiplierRange(int channel) const -> Range const * {
      return selectedRange<4>(params[MultiplierRangeSwitch + channel], multiplierRanges);
    }
    ...
    auto operand(int channel) const -> float { return rotationOf(params[AmountKnob + channel]); }

    auto operation(int channel) const -> Operation {
      return static_cast<Operation>(positionOf(params[OperationSwitch + channel]));
    }

    void output(int channel, float voltage) { outputs[FuncOutput + channel].setVoltage(voltage); }
