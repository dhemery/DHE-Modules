...
public:
  explicit Func6Panel(rack::engine::Module *module) {
    ...
    for (auto row = 0; row < channelCount; row++) {
      ...
      addInput(
          Jack::input(slug, module, column1, portY, Controls::FuncInput + row));
      addParam(
          Knob::large(slug, module, column3, y, Controls::AmountKnob + row));
      addOutput(Jack::output(slug, module, column5, portY,
                             Controls::FuncOutput + row));

      auto *offsetRangePickList =
          picklist::button(slug, "offset-range", offsetRanges.size(), module,
                           column4, y, Controls::OffsetRangeSwitch + row);
      addParam(offsetRangePickList);
      popupMenus.push_back(offsetRangePickList->menu());

      auto *multiplierRangePickList = picklist::button(
          slug, "multiplier-range", multiplierRanges.size(), module, column4, y,
          Controls::MultiplierRangeSwitch + row);
      addParam(multiplierRangePickList);
      popupMenus.push_back(multiplierRangePickList->menu());

      auto const updateRangePickList =
          [offsetRangePickList, multiplierRangePickList](bool isMultiply) {
            if (isMultiply) {
      ...
      addParam(new OperatorSwitch{updateRangePickList, slug, module, column2, y,
