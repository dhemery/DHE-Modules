...
public:
  ...
  void process(ProcessArgs const & /*ignored*/) override {
    ...
    for (auto i = 0; i < 2; i++) {
      ...
      setOutputs(Controls::AndOutputs + i, Controls::NandOutputs + i, aAndB,
                 voltageOffset);
      setOutputs(Controls::OrOutputs + i, Controls::NorOutputs + i, aOrB,
                 voltageOffset);
      setOutputs(Controls::XorOutputs + i, Controls::XnorOutputs + i, aXorB,
                 voltageOffset);
      setOutputs(Controls::ImplicationOutputs + i,
                 Controls::NonimplicationOutputs + i, aImpliesB, voltageOffset);
      setOutputs(Controls::ConverseImplicationOutputs + i,
                 Controls::ConverseNonimplicationOutputs + i, bImpliesA,
                 voltageOffset);
    }
  }
