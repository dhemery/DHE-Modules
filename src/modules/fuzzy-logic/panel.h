...
public:
  explicit Panel(std::string const &slug, rack::engine::Module *module) {
    ...
    y = top + 2.F * dy;
    addOutput(Jack::output(slug, module, ab_outer_column, y,
                           Controls::AndOutputs + 0));
    addOutput(Jack::output(slug, module, ab_inner_column, y,
                           Controls::NandOutputs + 0));
    addOutput(Jack::output(slug, module, cd_inner_column, y,
                           Controls::NandOutputs + 1));
    addOutput(Jack::output(slug, module, cd_outer_column, y,
