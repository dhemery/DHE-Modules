...
public:
  StepSelector(Controls &controls, int stepCount)
      : controls{controls}, stepMask{stepCount - 1} {}
  ...
  auto successor(int current, bool isLooping) const -> int {
    return successor(current, controls.selectionStart(),
                     controls.selectionLength(), isLooping);
  }

private:
  auto isEnabled(int step) const -> bool { return controls.isEnabled(step); }

  auto successor(int current, int selectionStart, int selectionLength,
                 bool isLooping) const -> int {
    ...
    for (auto i = current + 1; i < selectionStart + selectionLength; i++) {
      ...
      if (isSelected(candidate, selectionStart, selectionEnd) &&
          isEnabled(candidate)) {
        return candidate;
      }
    }
  ...
  auto isSelected(int candidate, int selectionStart, int selectionEnd) const
      -> bool {
