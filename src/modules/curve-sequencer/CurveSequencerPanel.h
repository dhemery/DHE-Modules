...
public:
  SelectionKnob(std::function<void(int)> action,
      ...
      : Knob{moduleSvgDir, "knob-small", module, x, y, index},
        knobChangedTo{std::move(action)} {
    snap = true;
  }
...
public:
  CurveSequencerPanel(rack::engine::Module *module) {
    ...
    addParam(Toggle::button(slug, module, left + buttonPortDistance, runY,
                            Controls::RunButton));

    addInput(Jack::input(slug, module, left, loopY, Controls::LoopInput));
    addParam(Toggle::button(slug, module, left + buttonPortDistance, loopY,
                            Controls::LoopButton));
    ...
    addParam(new SelectionKnob(onSelectionStartChange, slug, module, left,
                               selectionY, Controls::SelectionStartKnob));

    auto const onSelectionEndChange = [endMarker](int length) {
      endMarker->setSelectionLength(length);
    };
    auto constexpr selectionLengthX = left + hp2mm(2.F);
    addParam(new SelectionKnob(onSelectionEndChange, slug, module,
                               selectionLengthX, selectionY,
                               Controls::SelectionLengthKnob));

    addInput(Jack::input(slug, module, left, gateY, Controls::GateInput));
    addParam(Button::momentary(slug, module, left + buttonPortDistance, gateY,
                               Controls::GateButton));

    addInput(Jack::input(slug, module, left, resetY, Controls::ResetInput));
    addParam(Button::momentary(slug, module, left + buttonPortDistance, resetY,
    ...
    auto constexpr enabledButtonY =
        enabledPortY - portRadius - buttonRadius - 1.F;

    auto const generateModeLabels = std::vector<std::string>{
        "curve", "hold", "sustain", "input", "chase", "level"};
    auto const advanceModeLabels =
    ...
    for (auto step = 0; step < N; step++) {
      ...
      addChild(rack::createLightCentered<ProgressLight>(
          mm2px(x, activeY), module, Controls::ProgressLights + step + step));

      auto *generateModeButton = picklist::button(
          slug, "generate-mode", generateModeLabels.size(), module, x,
          generateModeY, Controls::ModeSwitches + step);
      addParam(generateModeButton);
      popupMenus.push_back(generateModeButton->menu());

      auto *advanceModeButton = picklist::button(
          slug, "advance-mode", advanceModeLabels.size(), module, x,
          advanceModeY, Controls::ConditionSwitches + step);
      addParam(advanceModeButton);
      popupMenus.push_back(advanceModeButton->menu());

      addParam(
          Knob::small(slug, module, x, levelY, Controls::LevelKnobs + step));

      addParam(Toggle::stepper(2, slug, module, x, shapeY,
                               Controls::ShapeSwitches + step));
      addParam(
          Knob::small(slug, module, x, curveY, Controls::CurveKnobs + step));

      addParam(Knob::small(slug, module, x, durationY,
                           Controls::DurationKnobs + step));

      addParam(Toggle::button(slug, module, x, enabledButtonY,
                              Controls::EnabledButtons + step));
      addInput(Jack::input(slug, module, x, enabledPortY,
    ...
    addInput(
        Jack::input(slug, module, right, eosY, Controls::CurveSequencerInput));

    addParam(Toggle::stepper(2, slug, module, right, levelY,
                             Controls::LevelRangeSwitch));
    addParam(Toggle::stepper(3, slug, module, right, durationY,
                             Controls::DurationRangeSwitch));
    addOutput(Jack::output(slug, module, right, outY,
