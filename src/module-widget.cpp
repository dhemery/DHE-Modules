...
auto ModuleWidget::create_button(std::string type, int index, rack::Vec center)
  ...
  auto off_image_file{std::string("res/") + module_name_ + "/button-" + type +
                      "-off.svg"};
  auto on_image_file{std::string("res/") + module_name_ + "/button-" + type +
                     "-on.svg"};
...
auto ModuleWidget::create_knob(std::string size, int index, rack::Vec center,
  ...
  auto knob_widget{rack::ParamWidget::create<KnobWidget>({0, 0}, module, index,
                                                         0.f, 1.f, initial)};
  auto image_file{std::string("res/") + module_name_ + "/knob-" + size +
                  ".svg"};
...
auto ModuleWidget::create_port(rack::Port::PortType type, int index,
  ...
  auto port_widget{rack::Port::create<PortWidget>({0, 0}, type, module, index)};
...
auto ModuleWidget::create_switch(int index, rack::Vec center, int max_position,
  ...
  auto low_image_file{std::string("res/") + module_name_ + "/switch-" + type +
                      "-low.svg"};
  switch_widget->addFrame(
      rack::SVG::load(rack::assetPlugin(plugin, low_image_file)));
  if (max_position == 2) {
    auto mid_image_file{std::string("res/") + module_name_ + "/switch-" + type +
                        "-mid.svg"};
    switch_widget->addFrame(
        rack::SVG::load(rack::assetPlugin(plugin, mid_image_file)));
  }
  auto high_image_file{std::string("res/") + module_name_ + "/switch-" + type +
                       "-high.svg"};
  switch_widget->addFrame(
