...
struct UpstageWidget : public ModuleWidget<UpstageWidget, Upstage> {
  ...
  explicit UpstageWidget(Upstage *module)
    ...
    install(column_2, y, large_knob(Upstage::LEVEL_KNOB));
    ...
    y += dy;
    install_button("normal", Upstage::WAIT_BUTTON, {column_1, y});
    install_button("normal", Upstage::TRIGGER_BUTTON, {column_3, y});

    y = 82.f;
    dy = 15.f;

    install(column_1, y, input_jack(Upstage::WAIT_IN));

    y += dy;
    install(column_1, y, input_jack(Upstage::TRIGGER_IN));
    install(column_3, y, output_jack(Upstage::TRIGGER_OUT));

    y += dy;
    install(column_3, y, output_jack(Upstage::MAIN_OUT));
  }
};
} // namespace DHE
