 ...
 */
static inline void configKnob(rack::engine::Module *module, int knobId,
                              std::string const &knobName,
                              std::string const &units, Range const &range,
                              float initialRotation = centeredRotation) {
  module->configParam(knobId, 0.F, 1.F, initialRotation, knobName, units, 0.F,
 ...
 */
static inline void configPercentageKnob(rack::engine::Module *module,
                                        int knobId, std::string const &knobName,
                                        Range const &range = rotationRange) {
  module->configParam(knobId, 0.F, 1.F, 0.5F, knobName, "%", 0.F,
 ...
 */
static inline void configAttenuator(rack::engine::Module *module, int knobId,
                                    std::string const &knobName) {
 ...
 */
static inline void configAttenuverter(rack::engine::Module *module, int knobId,
                                      std::string const &knobName) {
 ...
 */
static inline void configGain(rack::engine::Module *module, int knobId,
                              std::string const &knobName) {
...
public:
  void setFrameNames(std::array<std::string, N> const &names) {
    frameNames = names;
  }
...
template <int N>
void configFrameWidgetStates(rack::engine::Module *module, int paramId,
                             std::string const &paramName,
                             std::array<std::string, N> const &stateNames,
                             int initialState) {
  module->configParam<FrameWidgetParamQuantity<N>>(
      paramId, 0.F, (float)(N - 1), (float)initialState, paramName);
  auto const controlDisplay = dynamic_cast<FrameWidgetParamQuantity<N> *>(
 ...
 * Configures the param and display for a momentary button.
 */
static inline void configButton(rack::engine::Module *module, int buttonId,
                                std::string const &buttonName,
                                std::array<std::string, 2> const &stateNames,
                                int initialState) {
  configFrameWidgetStates<2>(module, buttonId, buttonName, stateNames,
                             initialState);
}
 ...
 */
template <int N>
void configToggle(rack::engine::Module *module, int toggleId,
                  std::string const &toggleName,
                  std::array<std::string, N> const &stateNames,
                  int initialState) {
  configFrameWidgetStates<N>(module, toggleId, toggleName, stateNames,
                             initialState);
}
