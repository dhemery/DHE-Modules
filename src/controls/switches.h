...
struct Selector {
  ...
  using TAction = std::function<void(typename TItems::ValueType)>;

  template <typename TPanel, typename TItems>
  struct Widget : public rack::app::SvgSwitch {
    using TValue = typename TItems::ValueType;

    Widget() {
      auto const panel_prefix = std::string{TPanel::svg_dir} + "/";
      for (auto const &frame_name : TItems::frame_names()) {
        addFrame(load_svg(panel_prefix + frame_name));
      }
      shadow->opacity = 0.F;
    }

    void set_action(TAction<TItems> const &action) { action_ = action; }

    void onChange(const rack::event::Change &e) override {
      rack::app::SvgSwitch::onChange(e);
      auto const position =
          static_cast<int>(this->getParamQuantity()->getValue());
      auto const selection = TItems::items()[position];
      action_(selection);
    }

  private:
    TAction<TItems> action_ = [](TValue) {};
  };

  template <typename TItems, typename TPanel>
  static inline auto install(
      TPanel *panel, int id, float xmm, float ymm,
      TAction<TItems> const &action = [](typename TItems::ValueType) {})
      -> Widget<TPanel, TItems> * {
    auto *widget = rack::createParamCentered<Widget<TPanel, TItems>>(
        mm2px(xmm, ymm), panel->getModule(), id);
    widget->set_action(action);
    panel->addParam(widget);
    return widget;
  ...
  static inline void config(TModule *module, int id, std::string const &name,
                            int selection = TItems::selection) {
    auto const max_value = static_cast<float>(TItems::items().size() - 1);
    auto const default_value = static_cast<float>(selection);
    module->configSwitch(id, 0.F, max_value, default_value, name,
                         TItems::labels());
  }
};
} // namespace dhe
