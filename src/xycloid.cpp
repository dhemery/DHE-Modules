...
struct Xycloid : rack::Module {
  ...
  const Knob throb_speed_knob =
      Knob::with_av(this, THROB_SPEED, THROB_SPEED_CV, THROB_SPEED_AV);
  const Knob wobble_depth_knob =
      Knob::with_av(this, WOBBLE_DEPTH, WOBBLE_DEPTH_CV, WOBBLE_DEPTH_AV);
  const Knob wobble_ratio_knob =
      Knob::with_av(this, WOBBLE_RATIO, WOBBLE_RATIO_CV, WOBBLE_RATIO_AV);

  const Knob x_gain_knob = Knob::with_cv(this, X_GAIN, X_GAIN_CV);
  const Switch<float> x_offset = Switch<float>::two(this, X_RANGE, 0.f, 1.f);

  const Knob y_gain_knob = Knob::with_cv(this, Y_GAIN, Y_GAIN_CV);
