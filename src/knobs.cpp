...
struct KnobsButtonNormal : rack::SVGSwitch, rack::ToggleSwitch {
  KnobsButtonNormal() {
    addFrame(rack::SVG::load(rack::assetPlugin(plugin, "res/knobs/button-normal-off.svg")));
    addFrame(rack::SVG::load(rack::assetPlugin(plugin, "res/knobs/button-normal-on.svg")));
  }
};

struct KnobsKnobLarge : rack::RoundKnob {
  KnobsKnobLarge() {
    setSVG(rack::SVG::load(rack::assetPlugin(plugin, "res/knobs/knob-large.svg")));
    shadow->opacity = 0.f;
  }
};
...
struct KnobsKnobTiny : rack::RoundKnob {
  KnobsKnobTiny() {
    setSVG(rack::SVG::load(rack::assetPlugin(plugin, "res/knobs/knob-tiny.svg")));
    shadow->opacity = 0.f;
  }
};
...
struct KnobsPort : rack::SVGPort {
  KnobsPort() {
    background->svg = rack::SVG::load(assetPlugin(plugin, "res/knobs/port.svg"));
    background->wrap();
...
struct KnobsSwitch2 : rack::SVGSwitch, rack::ToggleSwitch {
  KnobsSwitch2() {
    addFrame(rack::SVG::load(rack::assetPlugin(plugin, "res/knobs/switch-2-low.svg")));
    addFrame(rack::SVG::load(rack::assetPlugin(plugin, "res/knobs/switch-2-high.svg")));
  }
};

struct KnobsWidget : public ModuleWidget {
  KnobsWidget(rack::Module *module) : ModuleWidget(module, 8, "res/knobs/panel.svg") {
...
rack::Model *modelKnobs = rack::Model::create<DHE::Knobs, DHE::KnobsWidget>("DHE-Modules", "Knobs", "Knobs", rack::UTILITY_TAG);
