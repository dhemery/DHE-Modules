...
namespace curve_sequencer_step_selector {
  ...
  TEST_SUITE("curve_sequencer::StepSelector") {
    ...
    TEST_CASE("finds successor above given step") {
    ...
    TEST_CASE("finds successor below given step if selection wraps") {
    ...
    TEST_CASE("selection end can be successor") {
    ...
    TEST_CASE("no successor if no selected step is enabled") {
    ...
    TEST_CASE("no successor if selection wraps and no selected step is enabled") {
    ...
    TEST_CASE("no successor if given step is below selection") {
    ...
    TEST_CASE("no successor if given step is above selection") {
      givenSelection(controls, 4, 3); // 0 1 2 3 [4 5 6] 7
    ...
    TEST_CASE("no successor if given step not included in wrapped selection") {
    ...
    TEST_CASE("no successor if given step is selection and and sequencer is not looping") {
    ...
    TEST_CASE("first returns selection start if selection start is enabled") {
    ...
    TEST_CASE("first returns successor of selection start if selection start is disabled") {
      auto constexpr selectionStart = 3;
    ...
    TEST_CASE("no first if no selected step is enabled") {
    ...
    TEST_CASE("first finds successor earlier in selection if sequencer is looping") {
      givenSelection(controls, 0, stepCount); // [0 1 2 3 4 5 6 7]
      auto constexpr step = 3;
    ...
    TEST_CASE("given step can be its own successor if sequencer is looping") {
      givenSelection(controls, 0, stepCount); // [0 1 2 3 4 5 6 7]
      auto constexpr step = 3;
