...
public:
  ...
  void register_tests(TestRegistrar add) override {
    add("enter() activates stage",
        test<GenerateMode>([](Tester &t, Controls &controls, PhaseTimer & /**/,
                              GenerateMode &mode) {
    ...
    add("execute(l,s) advances the timer",
        test<GenerateMode>([](Tester &t, Controls &controls, PhaseTimer &timer,
                              GenerateMode &mode) {
    ...
    add("execute(l,s) outputs curve voltage",
        test<GenerateMode>([](Tester &t, Controls &controls, PhaseTimer &timer,
                              GenerateMode &mode) {
          ...
          auto constexpr phase_delta = 0.1F; // Each step is 1/10 full duration
          auto constexpr sample_time = full_duration * phase_delta;
          controls.duration_ = full_duration;
    ...
    add("execute(l,s) resets curve before generating if latch rises",
        test<GenerateMode>([](Tester &t, Controls &controls, PhaseTimer &timer,
                              GenerateMode &mode) {
