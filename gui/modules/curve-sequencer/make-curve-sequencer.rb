...
def cs_position_marker(x:, y:, type:)
...
def make_curve_sequencer(steps)
  ...
  cs_position_marker(x: step_x - Light::DIAMETER, y: active_y, type: :start)
  cs_position_marker(x: step_x + step_dx * (steps - 1) + Light::DIAMETER, y: active_y, type: :end)
