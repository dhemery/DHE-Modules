...
a_box = input_button_port x: input_x, y: top + 0 * dy, label: 'A'
input_button_port x: input_x, y: top + 1 * dy, label: 'B'
input_button_port x: input_x, y: top + 2 * dy, label: 'C'
d_box = input_button_port x: input_x, y: top + 3 * dy, label: 'D'

output_port(x: output_x, y: top + 1 * dy, label: 'Q')
output_port(x: output_x, y: top + 2 * dy, label: '¬ Q')
...
table_cols = 5
table_left = i2mm(1.1)
table_right = @width - i2mm(0.8)
table_dx = (table_right - table_left) / table_cols
table_top = a_box.top
table_bottom = d_box.bottom
...
(2...table_rows).each do |row|
  ...
  pick_list x: option_x, y: option_y, name: 'truth-value', size: :large, options: %w[F T], selection: 2, width: 8
end

(0...table_cols - 1).each do |col|
  option_x = table_left + col * table_dx + table_dx / 2
  option_y = table_top + table_dy * 1.5
  pick_list x: option_x, y: option_y, name: 'truth-mode', options: %w[RISE FALL EDGE HIGH LOW], selection: 4, width: 8
end
